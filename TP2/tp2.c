...
void kmeans(){
    ...
    do{
        ...
        // Atribui um centroid a cada ponto
        #pragma omp parallel for schedule(guided)
        for(int i=0 ; i<N; i++){

            int index = points.centroid[i];
            float min = dist(centroids.x[index],centroids.y[index],points.x[i],points.y[i]);
            ...
            for(int j=0; j< K; j++){
                ...
                float newDist = dist(centroids.x[j], centroids.y[j], points.x[i] , points.y[i] );
